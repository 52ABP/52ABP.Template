    ...
    public class AuditLogAppService : YoyoCmsTemplateAppServiceBase, IAuditLogAppService
    {
        ...
        private IQueryable<AuditLogAndUser> CreateAuditLogAndUsersQuery(GetAuditLogsInput input)
        {
            var query = from auditLog in _auditLogRepository.GetAll()
                        .WhereIf(input.StartDate.HasValue, auditLog => auditLog.ExecutionTime >= input.StartDate)
                        .WhereIf(input.EndDate.HasValue, auditLog => auditLog.ExecutionTime <= input.EndDate)
            ...
            query = query
                ...
                .WhereIf(!string.IsNullOrWhiteSpace(input.Parameters), c => c.AuditLog.Parameters.Contains(input.Parameters))
